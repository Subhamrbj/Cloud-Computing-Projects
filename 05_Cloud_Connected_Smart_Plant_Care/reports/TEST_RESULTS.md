...
- **33 automated backend/simulator tests passed.** See test-results.xml and tests/ for individual cases.
...
- **Running-app control checks passed:** manual watering button, stop button, automatic low-soil start, target-moisture stop and alert acknowledgement. Selected event records are in browser-actions.json.
...
Run `python -m pytest -q --junitxml=reports/test-results.xml` in an installed environment. Run `npm ci`, `npm run build` and `npm audit` in frontend. Start the backend and simulator as in README, then exercise the recorded UI steps. Test assertions isolate their SQLite database from the actual demo database. Browser screenshots show synthetic data only.

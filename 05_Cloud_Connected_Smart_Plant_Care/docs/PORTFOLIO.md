...
git commit -m "Add Verdant cloud-connected smart plant care project"
...
git add .
git diff --cached --stat
git commit -m "Initialize Verdant smart plant cloud platform"
git branch -M main
...
I’m sharing Verdant, a cloud-ready smart plant care project that connects simulated IoT sensors to an authenticated API and a live React dashboard.

The Python simulator models drying soil, gradual environmental changes and moisture recovery after watering. The backend stores readings, evaluates plant-specific rules and applies cooldown, tank-level and command-duration safeguards. The dashboard brings together history charts, alerts, device status and manual controls.

The project runs locally without hardware and includes a PostgreSQL cloud configuration, deployment instructions, automated tests and an optional ESP32 reference implementation. The published demo data is synthetic; the cloud deployment and hardware integration are next validation steps.

This project helped me work through device identity, API design, persistent storage, automation, failure handling and reproducible delivery.

Source code: [add your actual repository URL]
Demo: [add a working hosted URL only after deployment]

#CloudComputing #IoT #Python #FastAPI #React #SmartAgriculture #BuildInPublic

Before posting, run and understand the code and adjust the first-person narrative to reflect your own work. Do not say you deployed it, validated hardware or achieved water savings unless you have done and measured those things. Mention development assistance if your course or portfolio policy requires it.
...
Verdant is a cloud-ready plant care platform connecting simulated sensors to authenticated APIs, persistent storage and a React dashboard.
...
0:25 — Sign in and show the three plants, sensor cards and history.
0:55 — Start accelerated simulation and watch soil dry through a threshold.

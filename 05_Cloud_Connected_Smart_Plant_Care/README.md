...
A full-stack plant monitoring application with private user gardens, realistic virtual sensors, automatic watering and persistent history. Visitors can try it directly in their browser—no hardware, terminal or shared password required.

![Plant monitoring dashboard](screenshots/01_Dashboard.png)

**Release 2.0 — public-use edition.** The application and deployment package are ready. No hosted URL is claimed until it is published to your hosting account. [Beginner deployment guide](docs/DEPLOYMENT.md) · [Test evidence](reports/TEST_RESULTS.md) · [LinkedIn copy](docs/LINKEDIN_POST.md)
...
The package matches a numbered folder inside your Cloud-Computing-Projects repository, and can also become a standalone repository. No invented brand name is used.
